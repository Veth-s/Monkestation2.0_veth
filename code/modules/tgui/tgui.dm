...
/datum/tgui/process(seconds_per_tick, force = FALSE)
	...
	if(QDELETED(src_object))
		to_chat(src, span_interface("SRC_OBJECT HAS BEEN QDEL'D"))
		close(can_be_suspended = FALSE)
		return
	if(QDELETED(host))
		to_chat(src, span_interface("HOST HAS BEEN QDEL'D"))
		close(can_be_suspended = FALSE)
		return
	if(QDELETED(user))
		to_chat(src, span_interface("USER HAS BEEN QDEL'D"))
		close(can_be_suspended = FALSE)
		return
	if(QDELETED(window))
		to_chat(src, span_interface("WINDOW HAS BEEN QDEL'D"))
		close(can_be_suspended = FALSE)
		return
	// Validate ping
